...
def train_model(X_train, y_train, units=15):
    ...
    # レイヤーを定義
    model = Sequential()
    model.add(LSTM(input_shape=input_size,units=units,dropout = 0.1,return_sequences=False))
    #model.add(LSTM(units=units,dropout = 0.1,return_sequences=False))
    ...
    model.fit(X_train, y_train,epochs=20, validation_split=0.3, verbose=2, shuffle=False)
    return model
